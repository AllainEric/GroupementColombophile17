...
if (!class_exists('oftnCommon')) {
	...
	class oftnCommon {
        ...
        public static $plugin_premium_class = 'options_for_twenty_nineteen_premium_class';
        public static $plugin_trial = true;

		public static function plugin_version() {

            return options_for_twenty_nineteen_class::$version;
        ...
        public static function upgrade_link() {
            ...
            return add_query_arg('url', (isset($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'], 'https://webd.uk/product/' . self::$plugin_text_domain . '-' . (self::$plugin_premium_class ? 'upgrade' : 'donation') . '/');
        ...
        public static function plugin_action_links($settings_link, $premium = false) {
            ...
            if (!get_option(self::$plugin_prefix . '_purchased')) {
                ...
                if ($premium) {
                    ...
                    $settings_links[] = '<a href="' . self::upgrade_link() . '" title="' . sprintf(__('Upgrade now to %s Premium', self::$plugin_text_domain), self::$plugin_name) . '" style="color: orange; font-weight: bold;">' . (self::$plugin_premium_class ? __('Upgrade', self::$plugin_text_domain) : __('Donate', self::$plugin_text_domain)) . '</a>';
...
?>
<p><?= __('Can you spare some change for a poor web developer? You can help him survive the winter, feed his children and keep him off the streets. His eyes are dim. he cannot see. He\'s just a poor old man. His legs are grey. His ears are gnarled. His eyes and legs are old and bent. His ears are grizzled ...', self::$plugin_text_domain); ?></p>
<p><a href="<?= self::upgrade_link(); ?>" title="<?= sprintf(__('Donate to %s', self::$plugin_text_domain), self::$plugin_name); ?>" class="button-primary"><?= sprintf(__('Donate to %s', self::$plugin_text_domain), self::$plugin_name); ?></a> <a href="#" id="<?= self::$plugin_prefix; ?>-already-paid" title="<?= __('Aleady Donated!', self::$plugin_text_domain); ?>" class="button-primary"><?= __('Aleady Donated!', self::$plugin_text_domain); ?></a></p>
</div>
